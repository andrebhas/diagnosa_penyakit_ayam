			...
			<ul style="display:none;">
				...
				foreach($getPenyakit as $key => $val){
					...
					<li>P(Class Penyakit = "<?php echo $val->KdPenyakit; ?>") = <?php echo $PPenyakit; ?>
						...
						<h6>Hasil Probabilitas "<?php echo $val->Penyakit; ?>" adalah <?php echo round($Probabilitas,4); ?></h6>					
					</li>
					<?php
			...
			$getData	= $this->db->query("select 
				*
				...
				if($getData->num_rows()>0){
					echo "<h4 style='margin-bottom: 1px;'>HASIL Diagnosa dari hasil nilai Probabilitas ".$getData->row()->Hasil." maka ayam terdiagnosa penyakit '".$getData->row()->Penyakit."'</h4><br>";
					echo $getData->row()->Deskripsi."<br>";
					echo $getData->row()->Solusi."<br>";
					$Foto = explode("],[",$getData->row()->Foto);
					for($ro=0; $ro<count($Foto); $ro++){
						$File	= str_replace(array("]","["),"",$Foto[$ro]);
						if(is_file("penyakit/".$File)){
						echo "<img src='".base_url("penyakit/".$File)."' width='200'>";
						}
					}
					echo "<br><a href='".site_url('diagnosa/detail_perhitungan/'.$this->uri->segment(3))."' class='btn btn-success'>Detail Perhitungan</a>";
